// Avatar persistence — The Bee's Knees has no set_profile tool, so the chosen
...
export const AVATAR_EVENT = "beesknees:avatar-changed";

function key(npub: string): string {
  return `beesknees:avatar:${npub}`;
}
