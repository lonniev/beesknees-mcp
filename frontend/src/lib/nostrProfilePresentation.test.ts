// Pure-logic tests for the Nostr profile panel's presentation.
//
// Ported from eXcalibur along with the panel itself (#378 there). The rules
// travel because they are about a keypair and a touchscreen, not about that
// app: a read-only control must not be `disabled`, because a disabled element
// fires no pointer events and hover does not exist on an iPad.
//
...
describe("How to set explainer copy (#378)", () => {
  it("states key ownership, client-side edit, and why nsec is not collected", () => {
    ...
    assert.match(body, /not by The Bee's Knees/i);
